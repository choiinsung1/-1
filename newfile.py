...
        elif give % 2 != 0:
            await count.edit(content = "븅신 꼴았져?...")
            await asyncio.sleep(0.5)
            await count.edit(content = "븅신 꼴았져?... 현재 돈: "+str(moneyA[idA.index(ID)])+"원")
        f = open("UserData.txt", "w") #저장
        for i in range(0,len(idA),1):
            f.write(str(idA[i])+","+str(moneyA[i])+","+str(timeA[i])+"\n")
        f.close()
...
        embed.add_field(name="도박", value="돈, 돈받기, 도박 <금액>, 올인, 랭킹", inline=True)
        await message.channel.send('', embed=embed)
...
client.run(NzY0ODU3NzA3OTc4ODE3NTU3X4MXPw.YvIwnJo3J1J1BBTX35JK0CVIrEY)
